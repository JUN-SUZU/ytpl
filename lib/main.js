...
const main = module.exports = async (linkOrId, options, rt = 3) => {
  ...
  const res = await request(ref, opts.requestOptions);
  if (res.statusCode !== 200) {
    console.warn(`Received status code ${res.statusCode} while fetching the playlist page, retrying... (${rt - 1} retries left)`);
  ...
  // If you have any trouble about data format, check the raw json in parsed.json and adjust the parsing logic accordingly
  ...
  // Parse videos
  parsed.json.contents.twoColumnBrowseResultsRenderer.tabs[0]
    .tabRenderer.content.sectionListRenderer.contents[0]
    .itemSectionRenderer.contents.forEach((x, index) => {
      if (x.lockupViewModel && index < opts.limit) {
        const rawVideoInfo = x.lockupViewModel;
        resp.items.push({
          id: rawVideoInfo.contentId,
          index: index + 1,
          title: rawVideoInfo.metadata.lockupMetadataViewModel.title.content,
          shortUrl: `https://www.youtube.com/watch?v=${rawVideoInfo.contentId}`,
          url: `https://www.youtube.com/watch?v=${rawVideoInfo.contentId}&list=${plistId}&index=${index + 1}`,
          author: {
            url: 'https://www.youtube.com' + rawVideoInfo.metadata.lockupMetadataViewModel.metadata.contentMetadataViewModel.metadataRows[0].metadataParts[0].text.commandRuns[0].onTap.innertubeCommand.browseEndpoint.canonicalBaseUrl,
            name: rawVideoInfo.metadata.lockupMetadataViewModel.metadata.contentMetadataViewModel.metadataRows[0]
              .metadataParts[0].text.content,
            channelID: rawVideoInfo.metadata.lockupMetadataViewModel.metadata.contentMetadataViewModel.metadataRows[0].metadataParts[0].text.commandRuns[0].onTap.innertubeCommand.browseEndpoint.browseId,
          }
        });
      }
    });
  ...
  const token = parsed.json.contents.twoColumnBrowseResultsRenderer.tabs[0].tabRenderer.content.sectionListRenderer.contents[1].continuationItemViewModel.continuationCommand.innertubeCommand.continuationCommand.token;
...
/**
 * FL: watch later or favorites playlist that is managed by system
 * PL: standard playlist created by user
 * UU: channel uploads playlist
 * LL: user's liked videos playlist which is managed by system
 * RDCLAK5uy_: album playlist (music.youtube.com)
 * OLAK5uy_: album playlist (music.youtube.com)
 * UC: channel id
 */
...
const toChannelList = async ref => {
  ...
  if (res.statusCode !== 200) {
    throw new Error(`Received status code ${res.statusCode} while fetching the channel page`);
  }
  const body = await res.body.text();
